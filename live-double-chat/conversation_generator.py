...
# Update to use RAG-DB directory
RAG_DB_PATH = "../RAG-DB"

def get_available_docs():
    """Dynamically get the list of available documentation files from RAG-DB."""
    try:
        if os.path.exists(RAG_DB_PATH):
            files = [f for f in os.listdir(RAG_DB_PATH) if os.path.isfile(os.path.join(RAG_DB_PATH, f))]
            return files
        else:
            print(f"Warning: RAG-DB directory not found at {RAG_DB_PATH}")
            # Empty list if directory not found
            return []
    except Exception as e:
        print(f"Error listing RAG-DB files: {e}")
        return []
...
def load_document(filename):
    """Load a document from the RAG-DB directory."""
    if filename in DOC_CONTENTS:
        return DOC_CONTENTS[filename]

    try:
        # Try to load from RAG-DB
