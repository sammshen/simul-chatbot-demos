...
# Path to RAG database directory
RAG_DB_PATH = "../RAG-DB"

# Automatically scan for RAG files
def get_rag_files():
    # Get all .txt, .pdf, and .html files in the RAG-DB directory
    txt_files = glob.glob(os.path.join(RAG_DB_PATH, "*.txt"))
    pdf_files = glob.glob(os.path.join(RAG_DB_PATH, "*.pdf"))
    html_files = glob.glob(os.path.join(RAG_DB_PATH, "*.html"))

    # Return just the filenames
    return [os.path.basename(f) for f in txt_files + pdf_files + html_files]

# Get available RAG files
RAG_FILES = get_rag_files()

# Log available files
logger.info(f"Found {len(RAG_FILES)} files in RAG-DB: {RAG_FILES}")

# Cache for loaded file contents
RAG_FILE_CONTENTS = {}

def load_rag_file(filename):
    """Load a file from the RAG-DB directory."""
    if filename in RAG_FILE_CONTENTS:
        return RAG_FILE_CONTENTS[filename]

    try:
        file_path = os.path.join(RAG_DB_PATH, filename)
        if not os.path.exists(file_path):
            logger.error(f"RAG file {filename} does not exist at {file_path}")
        ...
        # Cache the content
        RAG_FILE_CONTENTS[filename] = content
        logger.info(f"Loaded RAG file {filename} ({len(content)} bytes)")
        return content
...
class UserSession:
    ...
    def _build_system_prompt(self):
        """Build a system prompt using content from RAG files."""
        ...
        if not RAG_FILES:
            logger.error("No RAG files found. Please add files to the RAG-DB directory.")
        ...
        system_file = rand.choice(RAG_FILES)
        system_content = load_rag_file(system_file)
        ...
        user_files = [f for f in RAG_FILES if f != system_file]
        user_file = rand.choice(user_files) if user_files else system_file
        user_content = load_rag_file(user_file)
...
def main():
    ...
    # Check if RAG-DB directory exists
    if not os.path.exists(RAG_DB_PATH):
        logger.error(f"RAG-DB directory not found at {RAG_DB_PATH}. Please create it and add document files.")
        return

    # Check for required libraries based on file types
    has_pdf_files = any(f.endswith('.pdf') for f in RAG_FILES)
    has_html_files = any(f.endswith('.html') for f in RAG_FILES)
    ...
    # Check if we have any files
    if not RAG_FILES:
        logger.error(f"No files found in {RAG_DB_PATH}. Please add .txt, .pdf, or .html files to the RAG-DB directory.")
        return

    # Preload all RAG files
    logger.info(f"Preloading {len(RAG_FILES)} documents from RAG-DB...")
    for rag_file in RAG_FILES:
        load_rag_file(rag_file)
    ...
    with open(real_time_stats_file, 'w') as f:
        ...
        f.write(f"Using real documents from {RAG_DB_PATH} as context\n")
        f.write("Available documents: " + ", ".join(RAG_FILES) + "\n\n")
    ...
    try:
        with open(real_time_stats_file, "a") as f:
            ...
            if "ProductionStack" in summaries and "Other" in summaries:
                ...
                rs_df['tpot'] = ((rs_df['generation_time'] - rs_df['ttft']) / (rs_df['generation_tokens'] - 1)) * 1000

                ps_tpot_ms = ps_df['tpot'].replace([float('inf'), -float('inf'), np.nan], np.nan).dropna().mean()
                rs_tpot_ms = rs_df['tpot'].replace([float('inf'), -float('inf'), np.nan], np.nan).dropna().mean()

                # Calculate throughput
                ps_throughput = ps_df["generation_tokens"].sum() / (ps_df["finish_time"].max() - ps_df["launch_time"].min())
                rs_throughput = rs_df["generation_tokens"].sum() / (rs_df["finish_time"].max() - rs_df["launch_time"].min())
                ...
                ttft_diff = rs_ttft_ms - ps_ttft_ms
                ttft_pct = ((rs_ttft_ms / ps_ttft_ms) - 1) * 100 if ps_ttft_ms > 0 else 0

                tpot_diff = rs_tpot_ms - ps_tpot_ms
                tpot_pct = ((rs_tpot_ms / ps_tpot_ms) - 1) * 100 if ps_tpot_ms > 0 else 0

                itl_diff = rs_itl_ms - ps_itl_ms
                itl_pct = ((rs_itl_ms / ps_itl_ms) - 1) * 100 if ps_itl_ms > 0 else 0

                # For throughput, higher is better - calculate how much slower Other is
                throughput_diff = ps_throughput - rs_throughput
                throughput_pct = (1 - (rs_throughput / ps_throughput)) * 100 if ps_throughput > 0 else 0
                ...
                doc_counts = {doc: 0 for doc in RAG_FILES}
                total_sessions = 0
